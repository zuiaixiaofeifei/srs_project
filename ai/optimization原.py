...
target_position = np.array([0.75, 1.6]) #初始目标点
...
def trajectory(n, b):
    # 生成随机轨迹，n是机械臂数量，bå是轨迹点数量
    traj = []
    for _ in range(b):
        point = {'theta': np.random.rand(n), 't': np.random.rand()}
        traj.append(point)
    return traj
...
def decode_individual(individual, n):
    ...
    for i in range(b):
        point = {'theta': individual[2 * n * i: 2 * n * (i + 1):2],
                 't': individual[1:2]}
...
def fitness_function(individual):
    ...
    # 适应度函数，可根据具体问题进行调整
    total_fitness = 1 / (beta[0] * F_c + beta[1] * t_T)
...
# 初始化种群
def run ():
    population = np.array([encode_individual(trajectory(n, b), n) for _ in range(population_size)])
